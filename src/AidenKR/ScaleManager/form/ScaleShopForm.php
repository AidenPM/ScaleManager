...
class ScaleShopForm implements Form
{
    /** @var ScaleManager */
    ...
    public function jsonSerialize(): array
    {
        ...
        $content .= "\n";
        $content .= "§l§a[!] §r§f보유중인 크기 코인 : " . $this->getCoinCount($this->player, $this->plugin->getScaleCoin());
        $content .= "\n";
    ...
    public function handleResponse(Player $player, $data): void
    {
        ...
        $coin = $this->plugin->getScaleCoin();

        switch ($data) {
            case 0:
                if ($player->getInventory()->contains($coin)) {
                    if ($scale->data[strtolower($name)]["scale"] < 30) {
            ...
            case 1:
                if ($player->getInventory()->contains($coin)) {
                    if ($scale->data[strtolower($name)]["scale"] > 0.5) {
                        $coin->setCount($coin->getCount() - 1);
                        $player->setScale($scale->getScale($name));
